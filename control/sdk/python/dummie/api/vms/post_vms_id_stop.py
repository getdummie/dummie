...
def _get_kwargs(
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/vms/{id}/stop".format(id=quote(str(id), safe=""),),
    }
...
def sync_detailed(
    ...
    *,
    ...
    """ Stop a VM
     ...
     Shuts the guest down but leaves its disk on the host, so a start boots it again from the same state.
    This does NOT free the quota the VM holds -- only a destroy does. A TTL keeps running while a VM is
    stopped.
