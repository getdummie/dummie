...
const localRules = `# Written by dagent when this file is missing. It is never rewritten in place:
...
# destinations change.
#
# Total deny. dagent does not know what any guest is allowed to reach; only the
...
# drop ends the flow.
drop ip $HOME_NET any -> any any (msg:"dagent: deny all egress (no ruleset from the control server)"; sid:1000000; rev:1;)
`
...
func applySuricataRules(rules string) error {
  // Read here rather than passed in: `dagent connect` does not otherwise hold a
  ...
  cfg, err := loadConfig("")
  if err != nil {
    return fmt.Errorf("could not read the dagent config: %w", err)
  }
