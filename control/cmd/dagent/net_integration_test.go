...
//
//   go test -tags= -run Integration -v ./cmd/dagent   (as root, with DAGENT_INTEGRATION=1)
...
func requireIntegration(t *testing.T) {
  t.Helper()
  if os.Getenv("DAGENT_INTEGRATION") == "" {
    t.Skip("set DAGENT_INTEGRATION=1 to run integration tests")
  }
  if os.Geteuid() != 0 {
...
func setupFleet(t *testing.T, n int) []testVM {
  ...
  for i := 0; i < n; i++ {
    v := testVM{
      ns:   fmt.Sprintf("dagent-test-%d", i),
