...
func noTargets(ip, name string) db.ListVMNetworkTargetsByAgentRow {
  return db.ListVMNetworkTargetsByAgentRow{VMIP: ip, VMName: name, HostVMID: "vm-" + name}
}
...
func TestGenerateSuricataRulesBlanketDeniesWithoutANameRule(t *testing.T) {
  for _, tc := range []struct {
    ...
    {"no targets at all", []db.ListVMNetworkTargetsByAgentRow{noTargets("10.0.0.2", "alpha")}, true},
    {"addresses only",
      []db.ListVMNetworkTargetsByAgentRow{row("10.0.0.2", "alpha", "ip", "1.2.3.4", "tcp", "22")}, true},
    // Lookup-only grants no web access, so nothing it compiles to needs to see a
    // handshake either.
    {"lookup-only domain",
      []db.ListVMNetworkTargetsByAgentRow{row("10.0.0.2", "alpha", "domain", "example.com", "", "none")}, true},
    {"a domain on both web ports",
      []db.ListVMNetworkTargetsByAgentRow{row("10.0.0.2", "alpha", "domain", "example.com", "", "")}, false},
    {"a domain on one web port",
      []db.ListVMNetworkTargetsByAgentRow{row("10.0.0.2", "alpha", "domain", "example.com", "", "443")}, false},
    // One name-based allowance is enough to need the syn, whatever else is there.
    {"addresses and a domain", []db.ListVMNetworkTargetsByAgentRow{
      row("10.0.0.2", "alpha", "domain", "example.com", "", ""),
