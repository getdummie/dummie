-- 0004_vms: the VM registry, and host metrics on the agents that run them.

-- Host metrics reported by the agent over its socket. They live on the agent
...
-- One row per VM the control plane asked an agent to run. The row is written
...
--
-- vm_id is the agent's own short id and is empty until the agent reports back:
...
CREATE UNIQUE INDEX idx_vms_agent_vm_id ON vms (agent_id, vm_id) WHERE vm_id <> '';
