-- The recorded image configuration is what the container declared, but an admin
-- has to be able to correct it: an image whose entrypoint or user is wrong is
-- otherwise only fixable by rebuilding it under a new name. The identity and the
-- artifact stay permanent -- name, source, ref, object, file, size and digest
-- describe a tar that already exists and cannot be edited into something else.
