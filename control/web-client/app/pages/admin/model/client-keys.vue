...
useHead({ title: 'dummie — admin · client keys' })
...
async function load() {
  ...
  error.value = null
  try {
    const res = await authFetch(`/admin/client-keys?limit=${limit.value}&offset=${offset.value}`)
...
async function create() {
  ...
  try {
    ...
    const res = await authFetch('/admin/client-keys', {
      method: 'POST',
...
async function confirmRevoke() {
  ...
  try {
    const res = await authFetch(`/admin/client-keys/${toRevoke.value.id}/revoke`, { method: 'POST' })
    if (!res.ok) throw new Error((await readMessage(res)) || `HTTP ${res.status}`)
    toRevoke.value = null
    await load()
  }
...
async function confirmDelete() {
  ...
  actionError.value = null
  try {
    const res = await authFetch(`/admin/client-keys/${toDelete.value.id}`, { method: 'DELETE' })
    if (!res.ok) throw new Error((await readMessage(res)) || `HTTP ${res.status}`)
    toDelete.value = null
    await load()
  }
...
<template>
  <div class="mx-auto max-w-6xl px-4 py-12 sm:px-6">
    ...
    <div class="mt-8 flex items-end justify-between gap-4">
      <div>
        <p class="eyebrow mb-2 text-primary-text">// admin · client keys</p>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            Clients that already enrolled with it keep working — revoke them individually to cut them off.
          </DialogDescription>
        </DialogHeader>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            This also drops the record of which clients enrolled with it. This cannot be undone.
          </DialogDescription>
        </DialogHeader>
